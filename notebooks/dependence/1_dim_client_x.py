...
# DBTITLE 1,Tools
def elimina_columnas(df_data:pys_DataFrame, drop_column:list,
                     keep_column:Optional[List[str]])-> pys_DataFrame:
    """La Función tiene como tarea principal eliminar columnas de un DataFrame, también se puede
     salvar guardar ciertas columnas.
...
def columnas_repetidas(df_data_1:pys_DataFrame, df_data_2:pys_DataFrame,
                    columna: str = None)->list:
...
def union_tablas(ls_route:list, column_union: str, other_column: str, jump_table: str,
                   df_data_0:pys_DataFrame, ls_drop:Optional[List[str]]) -> pys_DataFrame:
    ...
    for ii in ls_route:
        ...
        if not ii == f"{ENV}.{jump_table}":
            expresion = [first(column,ignorenulls=True)
                         .alias(column) for column in df_table.columns if column not in [column_union]]
...
def one_customers(df_data_0:pys_DataFrame) -> pys_DataFrame:
    """Se eliminan los usuarios con más de una coinciendencia hablando de beneficiarios,
    :params df_data_0: DataFrame con los datos de interes"""
...
# MAGIC En las siguientes celdas se construye la tabla de __dim_client__, es importante mencionar  
# que hay diferentes versiones de la tabla por lo que sin tener documentación veraz que fundamente 
# la creación de esta nueva tabla, se evita eliminar columnas y se obta por unir todas y cada una 
# de las tablas por medio de **hash_id**, por ello esta versión de tabla tiene más de 100 columnas.
...
# MAGIC %md
# MAGIC Tablas varias, son tablas que se unen a la tabla base crendo la tabla dim_client, después 
# de unir todas las tablas se hace una expanción de la 
